...
local function refreshResources()
    ...
    for i = 0, numResources - 1 do
        ...
        if GetNumResourceMetadata(resource, 'resource_type') > 0 then
            ...
            local params = json.decode(GetResourceMetadata(resource,
                'resource_type_extra',
                0))
...
AddEventHandler('onResourceListRefresh', function() refreshResources() end)
...
AddEventHandler('onResourceStarting', function(resource)
    ...
    if num then
        for i = 0, num - 1 do
            ...
            if file then addMap(file, resource) end
        end
    end
    ...
    if maps[resource] then
        if getCurrentMap() and getCurrentMap() ~= resource then
            if doesMapSupportGameType(getCurrentGameType(), resource) then
                print("Changing map from " .. getCurrentMap() .. " to " ..
                    resource)
    ...
    elseif gametypes[resource] then
        if getCurrentGameType() and getCurrentGameType() ~= resource then
            print("Changing gametype from " .. getCurrentGameType() .. " to " ..
                resource)
...
function roundEnded() SetTimeout(50, handleRoundEnd) end
...
AddEventHandler('onResourceStop', function(resource)
    if resource == currentGameType then
        ...
        if currentMap then StopResource(currentMap) end
...
AddEventHandler('rconCommand', function(commandName, args)
    if commandName == 'map' then
        if #args ~= 1 then RconPrint("usage: map [mapname]\n") end
        ...
        if currentGameType == nil or
            not doesMapSupportGameType(currentGameType, args[1]) then
            ...
            if count == 1 then
                ...
                RconPrint('map ' .. args[1] .. ' does not support ' ..
                    currentGameType .. "\n")
            end
    ...
    elseif commandName == 'gametype' then
        if #args ~= 1 then RconPrint("usage: gametype [name]\n") end
...
function getCurrentGameType() return currentGameType end

function getCurrentMap() return currentMap end

function getMaps() return maps end
...
function changeGameType(gameType)
    ...
    if currentGameType then StopResource(currentGameType) end
...
function doesMapSupportGameType(gameType, map)
    if not gametypes[gameType] then return false end

    if not maps[map] then return false end

    if not maps[map].gameTypes then return true end
