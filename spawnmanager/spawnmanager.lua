...
AddEventHandler('getMapDirectives', function(add)
    ...
    add('spawnpoint', function(state, model)
        ...
        for i, sp in ipairs(spawnPoints) do
            -- if it matches...
            if sp.x == state.xyz[1] and sp.y == state.xyz[2] and sp.z ==
                state.xyz[3] and sp.model == state.model then
                -- remove it.
...
function addSpawnPoint(spawn)
    ...
    if not tonumber(spawn.model) then model = GetHashKey(spawn.model) end

    -- is the model actually a model?
    if not IsModelInCdimage(model) then error("invalid spawn model") end
...
function setAutoSpawn(enabled) autoSpawnEnabled = enabled end
...
function spawnPlayer(spawnIdx, cb)
    ...
    Citizen.CreateThread(function()
        ...
        SetEntityCoordsNoOffset(ped, spawn.x, spawn.y, spawn.z, false, false,
            false, true)

        NetworkResurrectLocalPlayer(spawn.x, spawn.y, spawn.z, spawn.heading,
            true, true, false)
